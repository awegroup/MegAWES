...
function status = Run_drag_curve()
%Run_drag_curve - Comparison to a rotated hanging cable, catenary curve
%                 (Drag only) 
%
% Inputs:
%    None
...
%                         initAllSimParams_WindCatenary.m
%                         distance2curve.m (Catenary_3D folder)
% Subfunctions: none
% MAT-files required: none
%
% Syntax:  status = Run_drag_curve()
%
...
[ENVMT,simInit, T] = initAllSimParams_WindCatenary(ENVMT, simInit, T); %#ok<*ASGLU>

%% Run Test Case
simOut = sim('tether_WindCatenary_R2019B.slx', 'SrcWorkspace', 'current');
...
% plot3(simInit.x_catenary, simInit.y_catenary, simInit.z_catenary,'-')
